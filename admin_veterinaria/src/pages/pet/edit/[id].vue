...
const edit = async () => {
    ...
    try {
        ...
        if (FILE_AVATAR.value) {
            form.append("imagen", FILE_AVATAR.value);
        }
...
const show = async () => {
    try {
        ...
        IMAGEN_PREVISUALIZADA.value = pet_selected.value.photo;
        console.log(pet_selected.value.photo)
...
<template>
    ...
    <v-col cols="12" class="text-center mt-2">
        <VBtn color="primary" size="large" class="px-8 text-subtitle-1" elevation="2" @click="edit">
            Editar mascota
        </VBtn>
    </v-col>
</template>
