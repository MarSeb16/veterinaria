...
if ($net_income_total_before == 0) {
  ...
  public function kpi_veterinarie_most_asigned(Request $request){
   ...
   public function kpi_total_bruto(Request $request) {
    ...
    // TOTAL BRUTO DEL MES ANTERIOR
    $month_before = Carbon::parse($year."-".$month."-01")->subMonth();

    $total_bruto_appointments_before = Appointment::whereYear("created_at",$month_before->format("Y"))
                                                ->whereMonth("created_at",$month_before->format("m"))->sum("amount");
    $total_bruto_vaccinations_before = Vaccination::whereYear("created_at",$month_before->format("Y"))
                                                ->whereMonth("created_at",$month_before->format("m"))->sum("amount");
    $total_bruto_surgeries_before = Surgerie::whereYear("created_at",$month_before->format("Y"))
                                            ->whereMonth("created_at",$month_before->format("m"))->sum("amount");
    $total_bruto_general_before = $total_bruto_appointments_before + $total_bruto_vaccinations_before + $total_bruto_surgeries_before;

    if ($total_bruto_general_before == 0) {
        $variation_percentage = 0;
    } else {
   ...
   public function kpi_report_for_servicies(Request $request){
    ...
    // Ingresos brutos y variación de citas
    $bruto_income_appoinments = Appointment::whereYear("created_at",$year)->whereMonth("created_at",$month)->sum("amount");
    $bruto_income_appoinments_before = Appointment::whereYear("created_at",$month_before->format("Y"))
        ->whereMonth("created_at",$month_before->format("m"))->sum("amount");
    $VPAppointments = 0;
    if($bruto_income_appoinments_before != 0){
   ...
   public function kpi_pets_most_payments(Request $request){
    ...
    $appointments_pet_payment = DB::table("appointment_payments")->whereNull("appointment_payments.deleted_at")
    ...
    $vaccination_pet_payment = DB::table("vaccination_payments")->whereNull("vaccination_payments.deleted_at")
    ...
    $surgerie_pet_payment = DB::table("surgerie_payments")->whereNull("surgerie_payments.deleted_at")
    ...
    $payments_appointment_before = 0;
    $payments_vaccination_before = 0;
    $payments_surgerie_before = 0;
