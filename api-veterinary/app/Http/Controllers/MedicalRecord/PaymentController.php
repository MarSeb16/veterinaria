    ...
    public function store(Request $request)
    {
        ...
        if($request->appointment_id){//cita medica
            ...
            if(($amount_total_pay + $amount) > $appointment->amount){
                return response()->json([
                    "message" => 403,
                    "message_text" => "El monto del pago que quieres registrar superar al monto de la deuda (".($appointment->amount-$amount_total_pay)." BS)",
                ]);
            }
            if(($amount_total_pay + $amount) == $appointment->amount){
        ...
        if($request->vaccination_id){//vacuna
            ...
            if(($amount_total_pay + $amount) > $vaccination->amount){
                return response()->json([
                    "message" => 403,
                    "message_text" => "El monto del pago que quieres registrar superar al monto de la deuda (".($vaccination->amount-$amount_total_pay)." BS)",
                ]);
            }
            if(($amount_total_pay + $amount) == $vaccination->amount){
        ...
        if($request->surgerie_id){//cirujía
            ...
            if(($amount_total_pay + $amount) > $surgerie->amount){
                return response()->json([
                    "message" => 403,
                    "message_text" => "El monto del pago que quieres registrar superar al monto de la deuda (".($surgerie->amount-$amount_total_pay)." BS)",
                ]);
            }
            if(($amount_total_pay + $amount) == $surgerie->amount){
    ...
    public function update(Request $request, string $id)
    {
        ...
        if($request->appointment_id){//cita medica
            ...
            if((($amount_total_pay - $amount_current) + $amount) > $appointment->amount){
                return response()->json([
                    "message" => 403,
                    "message_text" => "El monto del pago que quieres editar superar al monto de la deuda (".($appointment->amount-$amount_total_pay)." BS)",
                ]);
            }
            if((($amount_total_pay - $amount_current) + $amount)  == $appointment->amount){
                $appointment->update(["state_pay" => 3]);
        ...
        if($request->vaccination_id){//vacuna
            ...
            if((($amount_total_pay - $amount_current) + $amount) > $vaccination->amount){
                return response()->json([
                    "message" => 403,
                    "message_text" => "El monto del pago que quieres editar superar al monto de la deuda (".($vaccination->amount-$amount_total_pay)." BS)",
                ]);
            }
            if((($amount_total_pay - $amount_current) + $amount)  == $vaccination->amount){
                $vaccination->update(["state_pay" => 3]);
        ...
        if($request->surgerie_id){//cirujía
            ...
            if((($amount_total_pay - $amount_current) + $amount) > $surgerie->amount){
                return response()->json([
                    "message" => 403,
                    "message_text" => "El monto del pago que quieres editar superar al monto de la deuda (".($surgerie->amount-$amount_total_pay)." BS)",
                ]);
            }
            if((($amount_total_pay - $amount_current) + $amount)  == $surgerie->amount){
                $surgerie->update(["state_pay" => 3]);
